            ...
            >
              <div className="p-8">
                ...
                <div className="flex items-center justify-center gap-4 mb-8">
                  ...
                  <div className="w-12 h-12 rounded-full bg-pink-100 flex items-center justify-center text-pink-600 font-bold text-lg">P</div>
                </div>

                {/* Two VCF dropzones */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Your VCF */}
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors relative ${userFile ? 'border-blue-300 bg-blue-50/40' : 'border-slate-300 hover:bg-slate-50'}`}>
                    <input
                      type="file"
                    ...
                    />
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 ${userFile ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                      <Upload className="w-6 h-6" />
                    </div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-blue-500 mb-2">Your VCF</p>
                    {userFile ? (
                      ...
                      <>
                        <p className="text-slate-900 font-medium text-sm mb-0.5">Upload your VCF</p>
                        <p className="text-slate-400 text-xs">.vcf, .vcf.gz, .vcf.bgz</p>
                      </>
                    )}
                  </div>

                  {/* Partner VCF */}
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors relative ${partnerFile ? 'border-pink-300 bg-pink-50/40' : 'border-slate-300 hover:bg-slate-50'}`}>
                    <input
                      type="file"
                      ...
                      className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                    />
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 ${partnerFile ? 'bg-pink-100 text-pink-600' : 'bg-slate-100 text-slate-400'}`}>
                      <Upload className="w-6 h-6" />
                    </div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-pink-500 mb-2">Partner's VCF</p>
                    {partnerFile ? (
                      ...
                      <>
                        <p className="text-slate-900 font-medium text-sm mb-0.5">Upload partner's VCF</p>
                        <p className="text-slate-400 text-xs">.vcf, .vcf.gz, .vcf.bgz</p>
                      </>
                    )}
                  </div>
